...
export default function InspeksiPlazaPage() {
  ...
  useEffect(() => {
    const savedDraft = localStorage.getItem('draft_plaza');
    if (savedDraft) {
      try {
    ...
    const timer = setTimeout(() => {
      if (formData.namaPetugas1 || formData.platNomor || currentStep > 1) {
        try {
          ...
          localStorage.setItem('draft_plaza', JSON.stringify(draftData));
        } catch (error) {
  ...
  const clearDraftFromLocal = () => {
    localStorage.removeItem('draft_plaza');
  };

  const kelengkapanSaranaPlaza = [
  ...
  // Canvas Drawing Functions for Petugas 2
  ...
  const handleSubmit = async () => {
    try {
      ...
      const ttdPetugas1Uploaded = formData.ttdPetugas1 ? await uploadSignatureToMinio(formData.ttdPetugas1, 'ttd-petugas1-plaza') : null;
      const ttdPetugas2Uploaded = formData.ttdPetugas2 ? await uploadSignatureToMinio(formData.ttdPetugas2, 'ttd-petugas2-plaza') : null;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="space-y-4">
          {kelengkapanSaranaPlaza.map((item) => {
            const itemData = formData.kelengkapanSarana[item] || { ada: false, jumlah: "", kondisi: "BAIK" };
            return (
              <div
                key={item}
                className={`border-2 rounded-lg p-4 transition ${
                  itemData.ada ? "border-orange-500 bg-orange-50" : "border-gray-200"
                }`}
              >
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className="w-10 h-10 bg-orange-500 hover:bg-orange-600 text-white rounded-lg font-bold text-lg flex items-center justify-center transition"
                            >
                              +
                            </button>
                          </div>
          ...
          <div className="border-2 border-black mb-4" style={{ fontFamily: 'Arial, sans-serif' }}>
                ...
                <div key={index} className="grid grid-cols-12 border-b border-black text-xs">
                  ...
                  <div className="col-span-2 border-r border-black">
                    <div className="grid grid-cols-2 h-full">
                      <div className="p-2 text-center border-r border-black flex items-center justify-center text-green-600 font-bold text-lg">{data?.ada ? "✔" : ""}</div>
                      <div className="p-2 text-center flex items-center justify-center text-red-600 font-bold text-lg">{!data?.ada ? "✔" : ""}</div>
                    </div>
                  </div>
                  ...
                  <div className="col-span-3">
                    <div className="grid grid-cols-3 h-full text-center">
                      <div className="p-2 border-r border-black flex items-center justify-center text-green-600 font-bold text-lg">{data?.ada && data?.kondisi === "BAIK" ? "✔" : ""}</div>
                      <div className="p-2 border-r border-black flex items-center justify-center text-yellow-600 font-bold text-lg">{data?.ada && data?.kondisi === "RUSAK_RINGAN" ? "✔" : ""}</div>
                      <div className="p-2 flex items-center justify-center text-red-600 font-bold text-lg">{data?.ada && data?.kondisi === "RUSAK_BERAT" ? "✔" : ""}</div>
                    </div>
                  </div>
                </div>
              );
            ...
            <div className="border-b-2 border-black">
              <table className="w-full text-xs">
                ...
                <tbody>
                  <tr>
                    <td className="border-r border-black p-3 text-center text-black">{formData.masaBerlakuSTNK ? new Date(formData.masaBerlakuSTNK).toLocaleDateString('id-ID', { year: 'numeric', month: '2-digit', day: '2-digit' }) : "-"}</td>
                    <td className="border-r border-black p-3 text-center text-black">{formData.masaBerlakuKIR ? new Date(formData.masaBerlakuKIR).toLocaleDateString('id-ID', { year: 'numeric', month: '2-digit', day: '2-digit' }) : "-"}</td>
                    <td className="border-r border-black p-3 text-center text-black">{formData.masaBerlakuSIMPetugas1 ? new Date(formData.masaBerlakuSIMPetugas1).toLocaleDateString('id-ID', { year: 'numeric', month: '2-digit', day: '2-digit' }) : "-"}</td>
                    <td className="border-r border-black p-3 text-center text-black">{formData.masaBerlakuSIMPetugas2 ? new Date(formData.masaBerlakuSIMPetugas2).toLocaleDateString('id-ID', { year: 'numeric', month: '2-digit', day: '2-digit' }) : "-"}</td>
                    <td className="border-r border-black p-3 text-center text-black">{formData.tanggalService ? new Date(formData.tanggalService).toLocaleDateString('id-ID', { year: 'numeric', month: '2-digit', day: '2-digit' }) : "-"}</td>
                    <td className="p-3 text-center text-black">{formData.jumlahBBM ? `${formData.jumlahBBM} BAR` : "-"}</td>
                  </tr>
                </tbody>
              </table>
            </div>
              ...
              <div className="grid grid-cols-2 gap-12">
                <div className="text-center">
                  ...
                  <div className="p-4 min-h-[100px] flex items-center justify-center mb-3">
                    {formData.ttdPetugas1 ? <img src={formData.ttdPetugas1} alt="TTD" className="max-h-20" /> : <span className="text-gray-400 text-xs italic">( Tanda tangan )</span>}
                  </div>
                  <div className="border-t-2 border-black pt-2 font-bold text-black">{formData.namaPetugas1}</div>
                ...
                <div className="text-center">
                  ...
                  <div className="p-4 min-h-[100px] flex items-center justify-center mb-3">
                    {formData.ttdPetugas2 ? <img src={formData.ttdPetugas2} alt="TTD Petugas 2" className="max-h-20" /> : <span className="text-gray-400 text-xs italic">( Tanda tangan )</span>}
                  </div>
                  <div className="border-t-2 border-black pt-2 font-bold text-black">{formData.namaPetugas2 || "-"}</div>
            ...
            <div className="grid grid-cols-2 border-t-2 border-black">
              <div className="border-r-2 border-black p-6 text-center">
                ...
                <div className="p-4 min-h-[80px] flex items-center justify-center mb-3"><span className="text-gray-400 text-xs italic">( Tanda tangan )</span></div>
                <div className="border-t-2 border-black pt-2 text-sm"><div className="font-bold text-black">Manager Traffic</div></div>
              </div>
              <div className="p-6 text-center">
                <div className="text-xs mb-2 uppercase font-medium text-black">PT JJC</div>
                <div className="p-4 min-h-[80px] flex items-center justify-center mb-3"><span className="text-gray-400 text-xs italic">( Tanda tangan )</span></div>
                <div className="border-t-2 border-black pt-2 text-sm"><div className="font-bold text-black">NIK</div></div>
              </div>
            </div>
          </div>
